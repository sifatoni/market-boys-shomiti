...
export default function EditDepositPage() {
  ...
  useEffect(() => {
    async function load() {
      try {
        ...
        const d: DepositDetail = res.data;
        setDeposit(d);
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      setError(Array.isArray(msg) ? msg.join(' · ') : msg ?? err.message ?? 'Failed to update deposit.');
    } finally {
      setLoading(false);
    }
  }
    ...
    <div className="max-w-2xl mx-auto space-y-6">
      ...
      <div className="flex items-center gap-4">
        ...
        <div>
          ...
          <p className="text-sm text-zinc-500 mt-0.5">Update deposit details</p>
        </div>
      </div>
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-6">
          ...
          {success && (
            <div className="p-4 bg-emerald-500/10 border border-emerald-500/20 rounded-lg text-emerald-400 text-sm">
              Deposit updated successfully! Redirecting…
            </div>
          )}
            ...
            {fetching ? (
              ...
              <div className="flex items-center gap-3 bg-zinc-800/60 border border-zinc-700 rounded-lg px-4 py-2.5">
                <div className="w-8 h-8 rounded-full bg-indigo-600 flex items-center justify-center text-white text-xs font-bold shrink-0">
                  {deposit?.member?.fullName?.charAt(0)?.toUpperCase() ?? '?'}
                </div>
                <div>

...
export default function MemberDepositsPage() {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      const depRes = await api.get('/deposits', { params: { memberId: found.id, limit: 100 } });
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center gap-3">
        ...
        <h1 className="text-2xl font-bold text-white">আমার জমার ইতিহাস</h1>
      </div>

      <div className="bg-zinc-900 border border-zinc-800 rounded-xl overflow-hidden">
        {deposits.length === 0 ? (
          <div className="px-6 py-16 text-center text-zinc-500 text-sm">কোনো জমার রেকর্ড নেই</div>
        ) : (
          <table className="w-full text-sm">
            ...
            <tfoot>
              <tr className="border-t-2 border-zinc-700 bg-zinc-800/50">
                <td colSpan={2} className="px-6 py-3 text-sm font-semibold text-zinc-300">মোট জমা</td>
                <td className="px-6 py-3 text-right text-lg font-bold text-emerald-400">
                  {formatTaka(total)}
                </td>
